...
<body>
		...
		<%
			...
			List<Phone> arr = daodienthoai.getAll(conn);
			int start = 0 , end = 9;
			if(arr.size() < 9)
			{
				end = arr.size();
			...
			List<Phone> list = daodienthoai.getListByPage(arr, start, end);
			...
			<section class="main-content">
				...
				%>		
							...
							<div class="product-box">
								...
								<p><a href="Productdetail_Controller?action=phone&id=<%=phone.getIdphone() %>"><img src="data:image/jpg/png;base64,<%=phone.getHinhanh() %>" width="250" height="250" alt="" /></a></p>
								<a href="Productdetail_Controller?action=phone&id=<%=phone.getIdphone() %>" class="title"> <%=phone.getName() %></a><br/>
								<a href="Productdetail_Controller?action=phone&id=<%=phone.getIdphone() %>" class="category">VN/A <%=phone.getMemory() %>  GB</a>
				...
				%>  
						...
						<ul class="pagination">
							...
							%>
							<li> <a href="PhoneForward?start=<%=a%>&end=<%=b%>"><%=i%></a></li>
							<%
								}
							%>
						</ul>						
					...
					<div class="span3 col">
							...
							<div id="myCarousel" class="carousel slide">
								...
								<div class="item">
										<ul class="thumbnails listing-products">
											<li class="span3">
												<div class="product-box">										
													<p><a href="Productdetail_Controller?action=phone&id=<%=phone.getIdphone() %>"><img src="data:image/jpg;base64,<%=phone.getHinhanh() %>" width="250" height="250" alt="" /></a></p>
													<a href="Productdetail_Controller?action=phone&id=<%=phone.getIdphone() %>" class="title"> <%=phone.getName() %></a><br/>
													<a href="Productdetail_Controller?action=phone&id=<%=phone.getIdphone() %>" class="category">VN/A <%=phone.getMemory() %>  GB</a>
													<p class="price"> <%=phone.getGiatien1() %> </p>
												</div>
						...
						<div class="block">								
							...
							%>
								<li>
									<a href="Productdetail_Controller?action=phone&id=<%=phone.getIdphone() %>" title="">
										<img src="data:image/jpg;base64,<%=phone.getHinhanh() %>" width="250" height="250" alt="">
									</a>
									<a href="Productdetail_Controller?action=phone&id=<%=phone.getIdphone() %>"><%=phone.getName()%></a>
								</li>
								<%
								}%>
							</ul>
